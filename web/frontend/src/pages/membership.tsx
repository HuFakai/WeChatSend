...
type Order = { id: string; outTradeNo: string; subject: string; amountFen: number; status: 'PENDING' | 'PAID' | 'CLOSED' | 'FAILED' | 'REFUNDED'; qrDataUrl?: string; qrCode?: string | null; expiresAt: string; paidAt?: string | null; failureReason?:string };
...
export function MembershipPage() {
  ...
  useEffect(() => { void load(); }, []);
  useEffect(()=>{const id=params.get('orderId');if(id)void api<Order>('/alipay/orders/'+id).then(setOrder).catch(e=>setError(e.message));},[params]);
  ...
  return <div className="page-enter space-y-6">
    ...
    <Button asChild variant="outline"><Link to="/orders">查看全部订单 / 恢复支付 →</Link></Button>{error && <ErrorState message={error} />}
