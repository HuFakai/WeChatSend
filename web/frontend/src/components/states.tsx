...
export function LoadingState({ label = '正在加载', fullScreen = false }: { label?: string; fullScreen?: boolean }) {
  return <div className={`flex items-center justify-center gap-2 text-sm text-neutral-500 ${fullScreen ? 'min-h-screen bg-[#f6f6f3]' : 'min-h-48'}`}><LoaderCircle className="h-4 w-4 animate-spin" />{label}</div>;
}

export function EmptyState({ title, detail, action }: { title: string; detail: string; action?: { label: string; onClick: () => void } }) {
  return (
    <div className="flex min-h-60 flex-col items-center justify-center rounded-2xl border border-dashed border-neutral-300 bg-white px-6 text-center">
