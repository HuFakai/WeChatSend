import { BookOpenText, BookUser, Braces, Bot, ChevronRight, LayoutDashboard, LogOut, Menu, MessageSquareText, Radio, Send, Settings2, X } from 'lucide-react';
  ...
  const sidebar = (
    <>
      ...
      <nav className="flex-1 space-y-1 p-3">
        ...
        {isAdmin && <NavLink to="/admin" end onClick={() => setOpen(false)} className={({ isActive }) => cn('group flex items-center rounded-lg px-3 py-2.5 text-sm transition-colors', isActive ? 'bg-neutral-950 text-white' : 'text-neutral-600 hover:bg-neutral-100 hover:text-black')}><Settings2 className="mr-3 h-4 w-4" />平台配置<ChevronRight className="ml-auto h-3.5 w-3.5 opacity-0 transition-opacity group-hover:opacity-60" /></NavLink>}
      </nav>
